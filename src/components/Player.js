...
class Player extends Component {
  ...
  handleChange = (event) => {
    if (event.target.value !== NaN) {
      ...
      this.player.currentTime =
        (event.target.value / 100) * this.player.duration;
    } else {
  ...
  componentDidUpdate(prevProps, prevState) {
    if (prevProps !== this.props) {
      if (this.props.sameButtonClicked) {
        this.player.pause();
        this.setState({ isplaying: false });
      } else {
        if (this.props.isClicked !== this.state.currentIndex) {
          this.startPlayer();
    ...
    return (
      <>
        <div id="player" className="container-fluid mb-0 pb-0">
          <div
            className="d-flex"
            style={{ marginBottom: "0.1rem ", marginTop: "0rem" }}
          >
            <input
              className="he"
              type="range"
              defaultValue="0"
              value={this.state.progressbar != NaN ? this.state.progressbar : 0}
          ...
          <div className="row">
            <div
              className="col-xs-3 now-playing"
            ...
            >
              <i
                ...
                style={{
                  ...
                  padding: "-1px",
                  boxShadow: "inset 0px 3px 10px red ,inset 0px -3px 10px red ",
                }}
              ></i>
            </div>
          </div>
